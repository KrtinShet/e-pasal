...
db.products.createIndex({ storeId: 1, slug: 1 }, { unique: true });
...
```javascript
db.categories.createIndex({ storeId: 1, slug: 1 }, { unique: true });
...
db.customers.createIndex({ storeId: 1, email: 1 }, { unique: true, sparse: true });
...
```javascript
// Order lookup
db.orders.createIndex({ storeId: 1, orderNumber: 1 }, { unique: true });
...
db.inventory.createIndex({ storeId: 1, productId: 1, variantId: 1 }, { unique: true });
...
db.conversations.createIndex({ storeId: 1, channel: 1, externalId: 1 }, { unique: true });
...
db.customers
  .find({
    storeId: ObjectId('...'),
    $or: [{ email: /search/i }, { phone: /search/i }, { firstName: /search/i }],
  })
  .limit(10);
...
```javascript
// Explain query
db.orders.find({ storeId, status: 'pending' }).sort({ createdAt: -1 }).explain('executionStats');
