...
export function ShipmentTracking({ orderId }: { orderId: string }) {
  ...
  if (loading) {
    return (
      <div className="animate-pulse space-y-3">
        <div className="h-5 w-32 bg-[var(--color-surface)] rounded" />
        <div className="h-20 w-full bg-[var(--color-surface)] rounded" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-[var(--color-background)] rounded-xl border border-[var(--color-border)]/20 p-5">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
        ...
        <div>
          <span className="text-[var(--color-text-muted)]">Consignment ID</span>
          <p className="font-medium text-[var(--color-text-primary)] font-mono text-xs">
      ...
      {shipment.trackingUrl && (
        <a
          ...
          className="flex items-center gap-1 text-sm text-[var(--color-primary)] hover:underline mb-4"
        >
      ...
      {shipment.statusHistory.length > 0 && (
        <div className="border-t border-[var(--color-border)]/10 pt-4">
          <h4 className="text-xs font-medium text-[var(--color-text-muted)] mb-3 uppercase">
            Status Timeline
          </h4>
            ...
            {[...shipment.statusHistory].reverse().map((entry, i) => (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div
                    className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${i === 0 ? 'bg-[var(--color-primary)]' : 'bg-[var(--color-border)]'}`}
                  />
                  {i < shipment.statusHistory.length - 1 && (
                    <div className="w-px flex-1 bg-[var(--color-border)]/30 mt-1" />
                  )}
                </div>
                <div className="pb-3">
